...
export async function submitEligibilityProof(
  ...
  // Always use the local proof server for privacy.
  ...
  // Initialize the SDK proof provider — it talks to /check and /prove
  ...
  // The SDK proof provider is now initialized and ready.
  ...
  //
  // For now, we verify the proof server connection and report status.

  try {
    // Verify the proof server is reachable via the SDK's expected endpoints
    const healthCheck = await fetch(`${proofServerUrl}/health`);
    if (!healthCheck.ok) {
      throw new Error(`Proof server health check failed (${healthCheck.status})`);
    }
    console.log("[Proof] Proof server is healthy");
  } catch (e) {
    if (e instanceof TypeError && e.message.includes("fetch")) {
      throw new Error(
        `Cannot reach proof server at ${proofServerUrl}.\n\n` +
        "Start the Docker proof server:\n\n" +
        "  docker run -d --name midnight-proof-server -p 6300:6300 midnightnetwork/proof-server:latest\n\n" +
        "Then verify: curl http://localhost:6300/health"
      );
    }
    throw e;
  }
